...
Gallery mode hides the manual `Preview` and `Try Next Preview` actions because thumbnails load lazily in the gallery itself. It shows Gallery-only filter and sort combos for common preview states, cached-complete entries, and orders such as newest, largest body, largest image, cache index, and UUID. `Cached complete` means the meaningful header bytes plus body bytes cover the advertised image size; it can include header-only entries with body size `0` and is not a decode guarantee.
